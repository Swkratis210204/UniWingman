...
// Σε περίπτωση λάθους, θεωρούμε τον χρήστη απλό φοιτητή (false)
    ...
    private void proceedToNextScreen(boolean isAdmin) {
        showMessage("Σύνδεση επιτυχής.");
        Intent intent;

        if (!OnboardingActivity.isOnboardingDone(LoginActivity.this)) {
            // Δεν έχει κάνει onboarding
            intent = new Intent(LoginActivity.this, OnboardingActivity.class);
        } else {
            // Έχει κάνει onboarding, πάμε στο κεντρικό μενού
            intent = new Intent(LoginActivity.this, MainActivity.class);

            // Αν είναι Admin, στέλνουμε ένα κρυφό "μήνυμα" (Extra) στο MainActivity
            if (isAdmin) {
                intent.putExtra("OPEN_ADMIN_TAB", true);
            }
        }
